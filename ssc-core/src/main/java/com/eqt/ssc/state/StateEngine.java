...
/**
 * This is the shim that will wrap whatever I need to track as much as I can in
...
public class StateEngine {
	...
	protected static final DateTimeFormatter formatter = DateTimeFormat
			.forPattern("/YYYY/MM/dd/YYYY-MM-dd-HH-mm-ss-SSS");
	 ...
	 * 
	 * @param record
	 *            is a record that has new state compared to its last instance.
	 */
	public void writeNewRecord(SSCRecord record) {
		...
		while (tries-- > 0) {
			try {
				...
				// we are loading data zipped into S3, every byte costs after
				// all!
				...
				LOG.debug(fileName + " compressed size: " + out.size() + "/" + json.getBytes().length);

				// setting metadata so that we can view and access the object in
				// S3 correctly.
				...
				if(tries > 0)
					LOG.warn("hiccup trying to write object (" + record.key.accountId + " " + record.key.methodName + ") tries left " + tries,e);
				else
	...
	/**
	 * This is used for moving bulk file downloads into the logging S3 bucket.
	 * It still requires an account as it will still log underneath an accounts
	 * area.
	 * 
	 * @param account
	 *            account to associate file with
	 * @param key
	 *            object name to give file
	 * @param file
	 *            file to upload.
	 */
	public void writeFile(SSCKey record, String key, File file) {
		...
		while(tries-- > 0) {
			try {
				//the API says that any error on this will result in no object written, so can just try again.
				s3.putObject(bucketName, buildBasePath(record) + "/" + key, file);
				return;
			} catch(Throwable t) {
				if(tries > 0)
					LOG.warn("hiccup trying to write object (" + record.accountId + " " + record.methodName + ") tries left " + tries,t);
				else
	...
	/**
	 * When local copy does not have any record, hit up FileSystem to see if its
	 * there. TODO: make it switch between S3 and a given mount point?
	 * 
	 * @param newRecord
	 *            the key for your new record to find a previous record for.
	 * @return
	 */
	protected String getFromFS(SSCKey newRecord) {
		// storage format is:
		// SSCLogs/AccountID/region/call/year/month/day/timestamp
		// we add another '/' to make sure we dont collide with a path like
		// blabla when we just want bla.
